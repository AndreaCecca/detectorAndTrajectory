...
class DetectionEval:
    ...
    def __init__(self,
        ...
        self.pred_boxes, self.meta = load_prediction(nusc, self.result_path, self.cfg.max_boxes_per_sample, DetectionBox, verbose=self.verbose)
...
#                         boxes_pred_1,
    ...
    def calc_sample_crit_GOAL2_unique_param(self, 
        ...
        info, all_pkls, gtdist, preddist, gtxs, predxs, createdimages, gt_boxes, nusc_maps  = pkl_print_visualizations(self.gt_boxes, 
                                box_tmp,
                                listtoken, self.nusc,
                                nusc_maps, device,
                                nworkers=nworkers, bsz=16,
    ...
    def pkl_test_2(self,listtoken: List[str],
        ...
        info, all_pkls, gtdist, preddist, gtxs, predxs, worst_ixes, gt_boxes, nusc, nusc_maps, dataset = test_pkl_2(self.gt_boxes, box_tmp,
                                listtoken, self.nusc,
                                nusc_maps, device,
                                nworkers=nworkers, bsz=16,
...
#                                     boxes_pred_1=self.pred_boxes,
    ...
    def filter_boxes_confidence_criticalityGOAL3(self, pred_boxes, filtering_condition=None, conf_th: float =0.4, crit: float =0.8):
        """
        ...
        for ind, sample_token in enumerate(pred_boxes.sample_tokens):
            pred_boxes.boxes[sample_token] = [box for box in pred_boxes[sample_token] if filtering_condition(box.crit, box.detection_score, crit, conf_th)]
            counter=counter + len(pred_boxes.boxes[sample_token])
