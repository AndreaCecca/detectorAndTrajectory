...
def pkl_print_visualizations(gt_boxes, pred_boxes, sample_tokens, nusc,
    ...
    if plot_kextremes > 0:
        ...
        with torch.no_grad():
            ...
            gtdist_sig = gtdist.sigmoid()
            preddist = model(predxs.to(device))
        createdimages= analyze_plot(gtxs, predxs, gtdist_sig.cpu(), preddist.sigmoid().cpu(),
                     masks.cpu(), pkls=all_pkls, save_image=False)
    ...
    return info, all_pkls, gtdist, preddist, gtxs, predxs, createdimages
